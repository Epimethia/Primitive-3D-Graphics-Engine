...
ssAnimatedModel::ssAnimatedModel(std::string modelFilname, std::string texFilename) {
	...
	program = ShaderLoader::CreateProgram("Assets/Shaders/Model Shaders/ModelShader.vs", "Assets/Shaders/Model Shaders/ModelShader.fs");
...
bool ssAnimatedModel::loadMesh(std::string fileName){
	...
	startFrame = 31;
	endFrame = 50;
	animFps = static_cast<int>(m_pScene->mAnimations[0]->mChannels[0]->mNumPositionKeys / m_pScene->mAnimations[0]->mDuration);
...
void ssAnimatedModel::setShaderEffectVariables(float dt, Terrain* terrain){
	...
	glUniform3f(cameraPosLoc, Camera::GetPos().x, Camera::GetPos().y, Camera::GetPos().z);
}
